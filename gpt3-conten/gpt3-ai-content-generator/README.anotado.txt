...
=== AIP â€“ AI Chatbots, Content Writer & Forms (formerly AI Power) ===
Contributors: senols
...
**AI Power (AIP)** is the **complete AI plugin for WordPress** â€” a full set of **artificial intelligence tools** to transform your site. From **AI chatbot** and **content generation** to **image creation, automation, and AI training** on your own data, AIP gives you everything in one place, right inside your WordPress dashboard.

Our **"Bring Your Own API Key"** model lets you connect to top AI providers (OpenAI, Google Gemini, Microsoft Azure, OpenRouter, DeepSeek and Ollama). No hidden credits â€” you use your own account and control your costs.

[ðŸ“– Documentation & Guides](https://docs.aipower.org/)  

### Why Choose AIP?

* **All-in-One** â€“ Chatbot, AI Writer, AI Forms, Image Generator, Automation, WooCommerce AI tools, and more.
* **Train on Your Data** â€“ Build your own **AI knowledge base** from posts, pages, products, PDFs, or files.
* **Voice + Chat** â€“ Real-time voice agents and voice input for interactive AI experiences.
* **WooCommerce AI** â€“ Generate product descriptions, titles, SEO tags, and even sell AI tokens to customers.
* **Fast & Flexible** â€“ Works with OpenAI GPT-5/4o, Google Gemini & Imagen, Azure, Replicate, and others.
* **Secure** â€“ 100% hosted on your WordPress site. Your data stays with you.
...
#### ðŸ“ AI Forms
- Drag-and-drop **AI-powered forms** to process user input into useful outputs â€” from outlines to support replies.
- Connect forms to **web search** and your AI training data.

#### âš™ï¸ AI Automation Engine
...
- Convert text to image with **OpenAI DALLÂ·E 3, GPT-4o, Google Imagen, Replicate models**.
- Pull free stock images from **Pexels** or **Pixabay**.
- Works in posts, tasks, chatbot, and forms.

#### ðŸ“š AI Training / Vector Database
...
1. Install via Plugins â†’ Add New, or upload to `/wp-content/plugins/gpt3-ai-content-generator`.
2. Activate via the **Plugins** menu.
3. Go to **AIP â†’ Dashboard** and enter your API key for at least one provider (e.g., OpenAI).
...
We support **OpenAI** (GPT-5, GPT-4o, GPT-3.5, DALLÂ·E 3, etc.), **Google** (Gemini, Imagen), **Microsoft Azure OpenAI**, **OpenRouter**, **DeepSeek**, **Ollama** and **Replicate**.
...
AIP is **all-in-one** â€” instead of installing separate plugins for chatbots, content writing, AI forms, and WooCommerce AI, you get them all in one optimized toolkit with centralized settings.
...
- **Improved**: UI/UX improvements for the Content Assistant, the post list assistant modal, and the â€œAdd to Vector Storeâ€ modal.
...
- **Fixed**: Automated Tasks â†’ Enhance Existing Content â€” "Fields to Update" incorrectly checked "Title" when editing a task even if it was not selected during save. Checkbox states now persist correctly.
- **Fixed**: Chatbot settings â€” "Reasoning" parameter was not persisting and always reverted to "low" (default) after save.
- **Improved**: SDK updated.

= 2.3.43 =

- **Improved**: Block Editor formatting â€” structured outputs now insert as proper blocks (Headings, Paragraphs, Lists).
...
- **Added**: Popup hint text â€” you can now display a custom hint above the chatbot popup.
- **Added**: Custom typing indicator â€” you can replace the animated dots with your own text.
- **Added**: Option to set different chatbot popup icon sizes â€” you can choose from small, medium, large, or extra-large popup icon.
- **Added**: Index status â€” you can now see which posts are indexed directly from the post list screen.
...
- **Fixed**: AI Training â†’ Settings now keeps fields/taxonomies with special characters in their keys.
...
- **Added**: REST Logs Handler for accessing logs via the pluginâ€™s REST API, enabling external integrations.
- **Added**: One-click IP Block button on the logs page.
- **Improved**: AI Train â†’ Site Content now has a default â€œAll by Post Typeâ€ mode with perâ€‘type checkboxes, live counts (e.g., Posts 34, Pages 20), and better progress/stop controls. You can still switch to â€œPick Specificâ€ to select items per page.
- **Improved**: Performance, cache and UI improvements on the AI Train page.
- **Improved**: New dragâ€‘andâ€‘drop with multiâ€‘file queue for file uploads to Pinecone and Qdrant.
...
- **Added**: A new setting under *AI Train â†’ Settings* to show or hide the Index button on Post/Product lists.  
...
- **Added**: Auto-Delete Logs â€” automatically removes logs after a specified period using a WP-Cron job.
- **Improved**: Search UI on the AI Train page.
- **Fixed**: Removed the "temperature" parameter for OpenAIâ€™s gpt-5, o1, o3, and o4 models, as they do not support this parameter.

= 2.3.33 =

- **Added**: Support for the latest OpenAI gpt-5 models. You can use "Sync button" to load them.
- **Added**: New "Reasoning Effort" parameter for OpenAIâ€™s reasoning models.
...
- **Added**: Support for Googleâ€™s latest image models (Imagen 4.0 Preview and Imagen 4.0 Ultra Preview) across the Content Writer, Automated Tasks, Chatbot, and Image Generator modules.
- **Added**: Support for Azure OpenAI DALLÂ·E image generation models across all modules. You can now use Azure-deployed DALLÂ·E models for image generation in the Content Writer, Automated Tasks, Chatbot, and Image Generator.
- **Added**: A new â€œOne-timeâ€ frequency option for Automated Tasks in both the AutoGPT and Content Writer modules. Tasks set to â€œOne-timeâ€ will automatically pause after completion.
...
- **Fixed**: An issue in the Automated Tasks module where the â€œX Valueâ€ and â€œImage Sizeâ€ settings for Content Writing tasks were not correctly loaded when editing a task.
- **Fixed**: An issue in the Content Writer's â€œSingleâ€ generation mode where requesting multiple images from stock photo providers like Pexels or Pixabay resulted in duplicate images. It now fetches unique images.
- **Fixed**: An issue where the â€œRun Nowâ€ button for the â€œAuto-Reply to Commentsâ€ task did not process all eligible commentsâ€”only those created since the last scheduled run.
