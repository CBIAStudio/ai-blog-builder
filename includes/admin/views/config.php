...
// Formatos (UI). Destacada siempre panoramica; internas configurables (panoramica o banner).
...
echo '<p class="description">Configura los motores de IA para texto e imÃ¡genes.</p>';
echo '</div>';
...
$text_provider_logo = plugins_url('assets/images/providers/' . $text_provider . '.svg', CBIA_PRO_PLUGIN_FILE);
if (!file_exists(plugin_dir_path(CBIA_PRO_PLUGIN_FILE) . 'assets/images/providers/' . $text_provider . '.svg')) {
    $text_provider_logo = plugins_url('assets/images/providers/openai.svg', CBIA_PRO_PLUGIN_FILE);
}

$image_provider_data = $providers_list[$image_provider] ?? [];
$image_provider_label = (string)($image_provider_data['label'] ?? 'OpenAI');
$image_provider_logo = plugins_url('assets/images/providers/' . $image_provider . '.svg', CBIA_PRO_PLUGIN_FILE);
if (!file_exists(plugin_dir_path(CBIA_PRO_PLUGIN_FILE) . 'assets/images/providers/' . $image_provider . '.svg')) {
    $image_provider_logo = plugins_url('assets/images/providers/openai.svg', CBIA_PRO_PLUGIN_FILE);
}
...
echo '<select name="images_limit" class="abb-select" style="width:220px;">';
...
for ($i = 1; $i <= 3; $i++) {
    ...
    echo '<tr><th scope="row"><label>Formato interna ' . $i . '</label></th><td>';
    ...
    echo '</select>';
    echo '<button type="button" class="button cbia-prompt-btn cbia-prompt-inline-btn" data-type="internal" data-idx="' . $i . '">Editar prompt interna ' . $i . '</button>';
    echo '</div>';
    echo '<p class="description">Este formato aplica solo a la interna ' . $i . '. El prompt base ya esta optimizado.</p>';
    echo '</td></tr>';
}

echo '<tr><th scope="row"><label>CSS banner internas</label></th><td>';
...
echo '<p class="description">Se aplica a <code>img.cbia-banner</code>. Puedes ajustarlo para tu tema sin tocar codigo del plugin.</p>';
echo '</td></tr>';
