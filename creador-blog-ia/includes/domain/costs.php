            ...
            if ($action === 'calibrate_real') {
                ...
                if ($actual_eur > 0) {
                    ...
                    if ($sum && !empty($sum['eur_total'])) {
                        ...
                        cbia_costes_log("Calibración REAL aplicada: billing={$actual_eur}€, real_calc={$estimated}€, mult=" . number_format($suggested, 4, ',', '.'));
                        $notice = 'saved';
                    } else {
                        cbia_costes_log("Calibración REAL: sin datos suficientes para calcular.");
                        $notice = 'calc';
                    }
                }
            }
