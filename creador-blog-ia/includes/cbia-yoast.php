 ...
 *
 * Incluye:
 * - Hook cbia_after_post_created: recalcula metas + actualiza semáforo + reindex best-effort
 * - Mantenimiento desde pestaña Yoast (batch/offset):
 *   (1) Recalcular metas
 *   (2) Actualizar semáforo (scores SEO/Legibilidad)
 *   (3) Ambos
 * - Marcar posts antiguos como CBIA (_cbia_created=1) con filtros opcionales
 * - Log propio (no confundir con cbia_activity_log)
 *
 * Nota:
 * - El “semáforo” aquí se rellena guardando:
 *   _yoast_wpseo_linkdex (SEO score)
 *   _yoast_wpseo_content_score (Legibilidad)
 *   con una heurística razonable para que deje de verse en gris.
...
if (!function_exists('cbia_yoast_update_semaphore_scores')) {
	...
	function cbia_yoast_update_semaphore_scores($post_id, $force = false) {
		...
		try {
			...
			cbia_yoast_log("WPSEO_Meta::save_postdata falló en {$post_id}: " . $e->getMessage());
		}
...
if (!function_exists('cbia_yoast_run_batch')) {
	...
	function cbia_yoast_run_batch($action, $batch = 50, $force = false, $only_cbia = true, $offset = 0) {
		...
		if (empty($ids)) {
			cbia_yoast_log("No hay posts para procesar (action={$action}, only_cbia=" . ($only_cbia ? 'sí' : 'no') . ", offset={$offset}).");
		...
		foreach ($ids as $post_id) {
			...
			if ($action === 'metas' || $action === 'both') {
				cbia_yoast_log("METAS: post {$post_id} '{$title}' (force=" . ($force ? 'sí' : 'no') . ")");
				$did = cbia_yoast_recalc_metas($post_id, $force);
				if ($did) $metas_changed++;
			}

			if ($action === 'semaphore' || $action === 'both') {
				cbia_yoast_log("SEMÁFORO: post {$post_id} '{$title}' (force=" . ($force ? 'sí' : 'no') . ")");
				list($did_scores, $seo, $read) = cbia_yoast_update_semaphore_scores($post_id, $force);
				if ($did_scores) {
					$scores_changed++;
					cbia_yoast_log("SEMÁFORO: scores guardados => SEO={$seo} | LEG={$read}");
				} else {
					cbia_yoast_log("SEMÁFORO: ya tenía scores (o no se pudo calcular).");
				}
			}
...
if (!function_exists('cbia_yoast_on_post_created')) {
	function cbia_yoast_on_post_created($post_id, $title = '', $content_html = '', $usage = array(), $model_used = '') {
		...
		// Esto es lo que te faltaba: al crear, actualizar semáforo inmediato
		cbia_yoast_log("HOOK: post creado {$post_id}. Recalculando metas + semáforo...");
		...
		cbia_yoast_log("HOOK: post {$post_id} metas=" . ($did_metas ? 'actualizadas' : 'ok') .
			" | semáforo=" . ($did_scores ? "OK (SEO={$seo}, LEG={$read})" : 'sin cambios') .
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {
			...
			if (!empty($post_unslashed['cbia_yoast_action']) && check_admin_referer('cbia_yoast_nonce_action', 'cbia_yoast_nonce')) {
				...
				if ($action === 'clear_log') {
					...
					cbia_yoast_log("MARCAR legacy: batch={$batch} offset={$offset} from=" . ($from ?: '(none)') . " to=" . ($to ?: '(none)') . " only_signals=" . ($sig ? 'sí' : 'no'));
