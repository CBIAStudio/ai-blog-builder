 ...
 * v11 (FIX: cálculo REAL por llamada y por modelo + SEO incluido)
 *
...
if (!function_exists('cbia_costes_calc_real_for_post')) {
    ...
    function cbia_costes_calc_real_for_post($post_id, $cost_settings) {
        ...
        foreach ($rows as $r) {
            ...
            // Si no tenemos modelo en tabla, no podemos calcular esa fila
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (!empty($u['cbia_form']) && $u['cbia_form'] === 'costes_actions' && check_admin_referer('cbia_costes_actions_nonce')) {
                ...
                if ($action === 'calc_last') {
                    ...
                    if ($sum) {
                        cbia_costes_log("Cálculo últimos {$n}: posts={$sum['posts']} real={$sum['real_posts']} est={$sum['est_posts']} real_calls={$sum['real_calls']} real_fails={$sum['real_fails']} total€=" . number_format((float)$sum['eur_total'], 4, ',', '.'));
                    } else {
